...
namespace GraphSharp.Controls
{
        ...
        where TGraph : class, IBidirectionalGraph<TVertex, TEdge>
    {
        ...
        public virtual void SaveLayout(XElement element)
        {
            if (element == null) throw new ArgumentNullException(nameof(element));

            var vertices = new XElement("vertices");

            foreach(var v in this._vertexControls.Values)
            {
                var vertex = new XElement("v");
                vertex.SetAttributeValue("i", v._Index);
                vertex.SetAttributeValue("n", v.Vertex);
                vertex.SetAttributeValue("r", v.Rect);
                vertices.Add(vertex);
            }
            element.Add(vertices);
        }
        public virtual void LoadLayout(XElement element)
        {
            if (element == null) throw new ArgumentNullException(nameof(element));
            var vertices = element.Element("vertices");
            if (vertices != null)
            {
                if (this.LayoutAlgorithm != null)
                {
                    this.LayoutAlgorithm.DynamicRouting = false;
                }

                var dict = this._vertexControls.ToLookup(v => v.Value._Index);
                foreach (var vertex in vertices.Elements())
                {
                    XAttribute r = null;
                    if (vertex.Name == "v"
                        && int.TryParse(vertex.Attribute("i").Value, out var vi)
                        && (r =vertex.Attribute("r"))!=null && r.Value!=null)
                    {
                        var rect = Rect.Parse(r.Value);

                        if (dict.Contains(vi))
                        {
                            foreach (var vc in dict[vi])
                            {
                                vc.Value.Rect = rect;
                            }
                        }
                ...
                if (this.LayoutAlgorithm != null)
                {
                    this.LayoutAlgorithm.DynamicRouting = true;
                }
            }
        }
        ...
        private IDictionary<TVertex, Point> GetRelativePositions(bool continueLayout)
        {
            ...
            else
            {
                foreach (var kvp in _vertexControls)
                {
                    ...
                    foreach (VertexControl vc in cvc.Vertices)
                    {
        ...
        protected void ChangeState(int stateIndex)
        {
            ...
            var dict = new Dictionary<TVertex, Rect>();
            ...
            if (positions != null)
            {
                ...
                {
                    if (!_vertexControls.TryGetValue(v, out VertexControl vp))
                        continue;

                    if (positions.TryGetValue(v, out pos))
                    {
                        RunMoveAnimation(vp, pos.X, pos.Y);
                        vp.Rect = new Rect(pos, new Size(vp.ActualWidth, vp.ActualHeight));
                        dict[v] = vp.Rect;
                        if(vp is CompoundVertexControl cp)
                        {
                            foreach(VertexControl xp in cp.Vertices)
                            {
                                if(xp.Vertex is TVertex t)
                                {
                                    dict[t] = xp.Rect;
                                }
                            }
            ...
            if (activeState.RouteInfos != null)
            {
                ...
                {
                    if (!_edgeControls.TryGetValue(e, out var ec))
                        continue;

                    if (activeState.RouteInfos.TryGetValue(e, out var routePoints))
                    {
            ...
            this.LayoutAlgorithm?.PostLayoutProcess(dict);
        }
