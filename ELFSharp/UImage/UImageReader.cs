...
public static class UImageReader
{
    ...
    public static UImage Load(string fileName) => Load(File.OpenRead(fileName), true);

    public static UImage Load(Stream stream, bool shouldOwnStream)
		=> TryLoad(stream, shouldOwnStream, out UImage result) switch
		{
    ...
    public static UImageResult TryLoad(string fileName, out UImage uImage) 
		=> TryLoad(File.OpenRead(fileName), true, out uImage);
    ...
    internal static uint ReadUInt32BigEndian(this BinaryReader reader) 
		=> (uint)IPAddress.HostToNetworkOrder(reader.ReadInt32());
