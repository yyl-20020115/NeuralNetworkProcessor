...
namespace SharpLua
{
    ...
    public partial class Lua
    {
        ...
        public static object LoadMem(LoadState S, Type t)
        {
            ...
            var bytes = new byte[str.chars.Length];
            for (int i = 0; i < str.chars.Length; i++)
                bytes[i] = (byte)str.chars[i];
            var pinnedPacket = GCHandle.Alloc(bytes, GCHandleType.Pinned);
            var b = Marshal.PtrToStructure(pinnedPacket.AddrOfPinnedObject(), t);
            pinnedPacket.Free();
            return b;
        }
        ...
        public static lu_byte LoadByte(LoadState S) => (lu_byte)LoadChar(S);
        public static object LoadVar(LoadState S, Type t) => LoadMem(S, t);
        public static object LoadVector(LoadState S, Type t, int n) => LoadMem(S, t, n);
        ...
        private static int LoadChar(LoadState S) => (char)LoadVar(S, typeof(char));
        ...
        private static lua_Number LoadNumber(LoadState S) => (lua_Number)LoadVar(S, typeof(lua_Number));
        ...
        private static TString LoadString(LoadState S)
        {
            ...
            {
                var s = luaZ_openspace(S.L, S.b, size);
