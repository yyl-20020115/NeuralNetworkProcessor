...
namespace SharpLua
{
    ...
    {
        public static int luaL_getn(lua_State L, int i) => (int)lua_objlen(L, i);
		...
		*/
        ...
        public static lua_Number luaL_opt(lua_State L, luaL_opt_delegate f, int n, lua_Number d)
            => lua_isnoneornil(L, (n != 0) ? d : f(L, n)) ? 1 : 0;
        ...
        public static lua_Integer luaL_opt_integer(lua_State L, luaL_opt_delegate_integer f, int n, lua_Number d) 
            => (lua_Integer)(lua_isnoneornil(L, n) ? d : f(L, (n)));
		...
		*/
        ...
        {
            public int p = 0;           /* current position in buffer */
            public int lvl = 0;  /* number of strings in the stack (level) */
            public lua_State L = null;
            public CharPtr buffer = new char[LUAL_BUFFERSIZE];
        }

        public static void luaL_addchar(luaL_Buffer B, char c)
        {
            if (B.p >= LUAL_BUFFERSIZE) luaL_prepbuffer(B);
            B.buffer[B.p++] = c;
        }
		...
		*/
        ...
        public static int abs_index(lua_State L, int i) => ((i) > 0 || (i) <= LUA_REGISTRYINDEX ? (i) : lua_gettop(L) + (i) + 1);
		...
		*/
        ...
        {
            var msg = lua_pushfstring(L, "%s expected, got %s",
                                              tname, luaL_typename(L, narg));
            return luaL_argerror(L, narg, msg);
        }


        private static void tag_error(lua_State L, int narg, int tag) => luaL_typerror(L, narg, lua_typename(L, tag));
        ...
        {
            var name = (def != null) ? luaL_optstring(L, narg, def) :
        ...
        public static CharPtr luaL_checklstring(lua_State L, int narg) => luaL_checklstring(L, narg, out var len);
        public static CharPtr luaL_checklstring(lua_State L, int narg, out uint len)
        {
            var s = lua_tolstring(L, narg, out len);
            if (s == null) tag_error(L, narg, LUA_TSTRING);
            return s;
        }


        public static CharPtr luaL_optlstring(lua_State L, int narg, CharPtr def) => luaL_optlstring(L, narg, def, out var len);
        ...
        public static lua_Number luaL_optnumber(lua_State L, int narg, lua_Number def) => luaL_opt(L, luaL_checknumber, narg, def);


        public static lua_Integer luaL_checkinteger(lua_State L, int narg)
        {
            var d = lua_tointeger(L, narg);
            if (d == 0 && lua_isnumber(L, narg) == 0)  /* avoid extra test when d is not 0 */
                tag_error(L, narg, LUA_TNUMBER);
            return d;
        }
        ...
        public static lua_Integer luaL_optinteger(lua_State L, int narg, lua_Integer def) => luaL_opt_integer(L, luaL_checkinteger, narg, def);
        ...
        public static void luaL_register(lua_State L, CharPtr libname, luaL_Reg[] l) => luaI_openlib(L, libname, l, 0);
		...
		*/
        ...
        private static int bufflen(luaL_Buffer B) => B.p;
        private static int bufffree(luaL_Buffer B) => LUAL_BUFFERSIZE - bufflen(B);
        ...
        {
            var L = B.L;
            var s = lua_tolstring(L, -1, out var vl);
            if (vl <= bufffree(B))
            {  /* fit into buffer? */
                var dst = new CharPtr(B.buffer.chars, B.buffer.index + B.p);
                var src = new CharPtr(s.chars, s.index);
                for (var i = 0; i < vl; i++)
                    dst[i] = src[i];
                B.p += (int)vl;
        ...
        public static int luaL_ref(lua_State L, int t)
        {
            ...
            var ref_ = (int)lua_tointeger(L, -1);  /* ref = t[FREELIST_REF] */
		...
		*/
        ...
        {
            var serr = strerror(errno());
            var filename = lua_tostring(L, fnameindex) + 1;
        ...
        public static int luaL_loadstring(lua_State L, CharPtr s) => luaL_loadbuffer(L, s, (uint)strlen(s), s);
        ...
        private static object l_alloc(Type t) => System.Activator.CreateInstance(t);
        ...
        {
            var L = lua_newstate(l_alloc, null);
