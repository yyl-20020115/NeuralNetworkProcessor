...
namespace SharpLua
{
    ...
    public partial class Lua
    {
		...
		*/
        public static int resetbits(ref lu_byte x, int m) => (x &= (lu_byte)~m);
        public static int setbits(ref lu_byte x, int m) => x |= (lu_byte)m;
        public static bool testbits(lu_byte x, int m) => (x & (lu_byte)m) != 0;
        public static int bitmask(int b) => 1 << b;
        public static int bit2mask(int b1, int b2) => (bitmask(b1) | bitmask(b2));
        public static int l_setbit(ref lu_byte x, int b) => setbits(ref x, bitmask(b));
        public static int resetbit(ref lu_byte x, int b) => resetbits(ref x, bitmask(b));
        public static bool testbit(lu_byte x, int b) => testbits(x, bitmask(b));
        public static int set2bits(ref lu_byte x, int b1, int b2) => setbits(ref x, (bit2mask(b1, b2)));
        public static int reset2bits(ref lu_byte x, int b1, int b2) => resetbits(ref x, (bit2mask(b1, b2)));
        public static bool test2bits(lu_byte x, int b1, int b2) => testbits(x, (bit2mask(b1, b2)));
		...
		*/
        ...
        public static bool iswhite(GCObject x) => test2bits(x.gch.marked, WHITE0BIT, WHITE1BIT);
        public static bool isblack(GCObject x) => testbit(x.gch.marked, BLACKBIT);
        public static bool isgray(GCObject x) => (!isblack(x) && !iswhite(x));

        public static int otherwhite(global_State g) => g.currentwhite ^ WHITEBITS;
        public static bool isdead(global_State g, GCObject v) => (v.gch.marked & otherwhite(g) & WHITEBITS) != 0;
        ...
        public static bool valiswhite(TValue x) => (iscollectable(x) && iswhite(gcvalue(x)));

        public static byte luaC_white(global_State g) => (byte)(g.currentwhite & WHITEBITS);
        ...
        public static bool isfinalized(Udata_uv u) => testbit(u.marked, FINALIZEDBIT);
        public static void markfinalized(Udata_uv u)
        {
            var marked = u.marked;  // can't pass properties in as ref
        ...
        private static int traversetable(global_State g, Table h)
        {
            ...
            var mode = gfasttm(g, h.metatable, TMS.TM_MODE);
		...
		*/
        private static void traverseproto(global_State g, Proto f)
        {
            ...
            for (var i = 0; i < f.sizek; i++)  /* mark literals */
                markvalue(g, f.k[i]);
            for (var i = 0; i < f.sizeupvalues; i++)
            {  /* mark upvalue names */
                if (f.upvalues[i] != null)
                    stringmark(f.upvalues[i]);
            }
            for (var i = 0; i < f.sizep; i++)
            {  /* mark nested protos */
                if (f.p[i] != null)
                    markobject(g, f.p[i]);
            }
            for (var i  = 0; i < f.sizelocvars; i++)
        ...
        private static void traversestack(global_State g, lua_State l)
        {
            ...
            markvalue(g, gt(l));
            var lim = l.top;
            for (var ci = l.base_ci[0]; ci <= l.ci; CallInfo.inc(ref ci))
            {
		...
		*/
        ...
        {
            var g = G(L);
            var o = g.tmudata.gch.next;  /* get first element */
            var udata = rawgco2u(o);
            ...
            g.mainthread.next = o;
            makewhite(g, o);
            var tm = fasttm(L, udata.uv.metatable, TMS.TM_GC);
            if (tm != null)
            {
		...
		*/
        ...
        {
            for (var uv = g.uvhead.u.l.next; uv != g.uvhead; uv = uv.u.l.next)
            {
        ...
        private static void atomic(lua_State L)
        {
            ...
            g.grayagain = null;
            propagateall(g);
            var udsize = luaC_separateudata(L, 0);  /* separate userdata to be finalized */
        ...
        {
            var g = G(L);
            var lim = (l_mem)((GCSTEPSIZE / 100) * g.gcstepmul);
            if (lim == 0)
