...
namespace SharpLua
{
    ...
    public partial class Lua
    {
		...
		*/
        private static int luaB_print(lua_State L)
        {
            ...
            for (int i = 1; i <= n; i++)
            {
                ...
                lua_call(L, 1, 1);
                var s = lua_tostring(L, -1);  /* get result */
                if (s == null)
        ...
        private static int luaB_tonumber(lua_State L)
        {
            ...
            {
                var s1 = luaL_checkstring(L, 1);
                luaL_argcheck(L, 2 <= base_ && base_ <= 36, 2, "base out of range");
                var n = strtoul(s1, out var s2, base_);
                if (s1 != s2)
		...
		*/
        ...
        {
            var cname = luaL_optstring(L, 2, "=(load)");
            luaL_checktype(L, 1, LUA_TFUNCTION);
            lua_settop(L, 3);  /* function, eventual name, plus one reserved slot */
            int status = lua_load(L, generic_reader, null, cname);
        ...
        private readonly static luaL_Reg[] base_funcs = {
          ...
          new("gcinfo", luaB_gcinfo),
          new("getfenv", luaB_getfenv),
          new("getmetatable", luaB_getmetatable),
          new("loadfile", luaB_loadfile),
          new("load", luaB_load),
          new("loadstring", luaB_loadstring),
          new("next", luaB_next),
