...
namespace SharpLua
{
    ...
    public partial class Lua
    {
        ...
        public static FilePtr tofilep(lua_State L) => (FilePtr)luaL_checkudata(L, 1, LUA_FILEHANDLE);
        ...
        {
            var pf = (FilePtr)lua_newuserdata(L, typeof(FilePtr));
        ...
        private static Stream getiofile(lua_State L, int findex)
        {
            ...
            var f = (lua_touserdata(L, -1) as FilePtr)?.file;
            if (f == null)
        ...
        private static int io_input(lua_State L) => g_iofile(L, IO_INPUT, "r");

        private static int io_output(lua_State L) => g_iofile(L, IO_OUTPUT, "w");
        ...
        private static int read_chars(lua_State L, Stream f, uint n)
        {
            ...
            var b = new luaL_Buffer();
            luaL_buffinit(L, b);
            uint rlen = LUAL_BUFFERSIZE;  /* try to read that much each time */
            do
            {
                var p = luaL_prepbuffer(b);
        ...
        private static int io_read(lua_State L) => g_read(L, getiofile(L, IO_INPUT), 1);
        private static int f_read(lua_State L) => g_read(L, tofile(L), 2);
        private static int io_readline(lua_State L)
        {
            var f = (lua_touserdata(L, lua_upvalueindex(1)) as FilePtr)?.file;
            if (f == null)  /* file is already closed? */
                luaL_error(L, "file is already closed");
            var sucess = read_line(L, f);
            if (ferror(f) != 0)
        ...
        private static int g_write(lua_State L, Stream f, int arg)
        {
            ...
            for (; (nargs--) != 0; arg++)
            {
                ...
                {
                    var s = luaL_checklstring(L, arg, out var l);
                    status = ((status != 0) && (fwrite(s, GetUnmanagedSize(typeof(char)), (int)l, f) == l)) ? 1 : 0;
                }
            }
            return pushresult(L, status, null);
        }
        private static int io_write(lua_State L) => g_write(L, getiofile(L, IO_OUTPUT), 1);
        private static int f_write(lua_State L) => g_write(L, tofile(L), 2);
        ...
        private static int f_setvbuf(lua_State L)
        {
            ...
            var sz = luaL_optinteger(L, 3, LUAL_BUFFERSIZE);
