...
namespace SharpLua
{
    public partial class Lua
    {
        ...
        {
            var f = fopen(filename, "r");  /* try to open file */
        ...
        private static CharPtr pushnexttemplate(lua_State L, CharPtr path)
        {
            ...
            var l = strchr(path, LUA_PATHSEP[0]);  /* find next separator */
        ...
        private static CharPtr findfile(lua_State L, CharPtr name, CharPtr pname)
        {
            ...
            {
                var filename = luaL_gsub(L, lua_tostring(L, -1), LUA_PATH_MARK, name);
        ...
        {
            var mark = strchr(modname, LUA_IGMARK[0]);
            if (mark != null) modname = mark + 1;
            var funcname = luaL_gsub(L, modname, ".", LUA_OFSEP);
        ...
        {
            var name = luaL_checkstring(L, 1);
            var filename = findfile(L, name, "cpath");
            if (filename == null) return 1;  /* library not found in this path */
            var funcname = mkfuncname(L, name);
        ...
        private static int loader_Croot(lua_State L)
        {
            ...
            var filename = findfile(L, lua_tostring(L, -1), "cpath");
        ...
        private static void modinit(lua_State L, CharPtr modname)
        {
            ...
            var dot = strrchr(modname, '.');  /* look for last dot in module name */
        ...
        {
            var modname = luaL_checkstring(L, 1);
