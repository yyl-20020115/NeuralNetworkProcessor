...
namespace SharpLua
{
    ...
    public partial class Lua
    {
        ...
        public static int getline(Proto f, int pc) 
            => (f.lineinfo != null) ? f.lineinfo[pc] : 0;
		...
		*/
        ...
        public static lua_Hook lua_gethook(lua_State L) => L.hook;


        public static int lua_gethookmask(lua_State L) => L.hookmask;


        public static int lua_gethookcount(lua_State L) => L.basehookcount;
        ...
        {
            var ci = L.base_ci[ar.i_ci];
            var name = findlocal(L, ci, n);
            lua_lock(L);
            if (name != null)
        ...
        private static void collectvalidlines(lua_State L, Closure f)
        {
            ...
            {
                var t = luaH_new(L, 0, 0);
                var lineinfo = f.l.p.lineinfo;
		...
		*/
        ...
        private static int checkjump(Proto pt, int pc) => !(0 <= pc && pc < pt.sizecode) ? 0 : 1;

        private static int checkreg(Proto pt, int reg) => !((reg) < (pt).maxstacksize) ? 0 : 1;
        ...
        public static int checkopenop(Proto pt, int pc) => luaG_checkopenop(pt.code[pc + 1]);
        ...
        private static Instruction symbexec(Proto pt, int lastpc, int reg)
        {
            ...
            int dest;
            var last = pt.sizecode - 1;  /* points to final return (a `neutral' instruction) */
        ...
        public static int luaG_checkcode(Proto pt) => (symbexec(pt, pt.sizecode, NO_REG) != 0) ? 1 : 0;


        private static CharPtr kname(Proto p, int c) => ISK(c) != 0 && ttisstring(p.k[INDEXK(c)]) ? svalue(p.k[INDEXK(c)]) : "?";


        private static CharPtr getobjname(lua_State L, CallInfo ci, int stackpos, ref CharPtr name)
        {
            if (isLua(ci))
            {  /* a Lua function? */
                var p = ci_func(ci).l.p;
                var pc = currentpc(L, ci);
                Instruction i;
                ...
                switch (GET_OPCODE(i))
                {
                    case OpCode.OP_GETGLOBAL:
                        {
                            var g = GETARG_Bx(i);  /* global index */
        ...
        {
            for (var p = ci.base_; p < ci.top; StkId.inc(ref p))
                if (o == p) return 1;
            return 0;
        }
        ...
        {
            CharPtr name = null;
            var t = luaT_typenames[ttype(o)];
            var kind = (isinstack(L.ci, o)) != 0 ?
        ...
        {
            var t1 = luaT_typenames[ttype(p1)];
            var t2 = luaT_typenames[ttype(p2)];
            if (t1[2] == t2[2])
        ...
        private static void addinfo(lua_State L, CharPtr msg)
        {
            ...
            if (isLua(ci))
            {  /* is Lua code? */
                var buff = new CharPtr(new char[LUA_IDSIZE]);  /* add file:line information */
        ...
        public static void luaG_errormsg(lua_State L)
        {
            ...
            {  /* is there an error handling function? */
                var errfunc = restorestack(L, L.errfunc);
