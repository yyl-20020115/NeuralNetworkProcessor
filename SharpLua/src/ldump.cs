...
namespace SharpLua
{
    ...
    public partial class Lua
    {
        ...
        {
            var array = b as Array;
            Debug.Assert(array.Length == n);
            for (int i = 0; i < n; i++)
                DumpMem(array.GetValue(i), D);
        }

        public static void DumpVar(object x, DumpState D) => DumpMem(x, D);
        ...
        private static void DumpChar(int y, DumpState D) => DumpVar((char)y, D);

        private static void DumpInt(int x, DumpState D) => DumpVar(x, D);
        ...
        private static void DumpCode(Proto f, DumpState D) => DumpVector(f.code, f.sizecode, D);
        ...
        private static void DumpConstants(Proto f, DumpState D)
        {
            ...
            n = f.sizep;
            DumpInt(n, D);
            for (int i = 0; i < n; i++) DumpFunction(f.p[i], f.source, D);
        }

        private static void DumpDebug(Proto f, DumpState D)
        {
            var n = (D.strip != 0) ? 0 : f.sizelineinfo;
            ...
            DumpInt(n, D);
            for (int i = 0; i < n; i++) DumpString(f.upvalues[i], D);
        }
