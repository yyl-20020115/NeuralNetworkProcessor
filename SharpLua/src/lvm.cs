...
namespace SharpLua
{
    ...
    {
        public static int tostring(lua_State L, StkId o) => ((ttype(o) == LUA_TSTRING) || (luaV_tostring(L, o) != 0)) ? 1 : 0;

        public static int tonumber(ref StkId o, TValue n) => ((ttype(o) == LUA_TNUMBER || (((o) = luaV_tonumber(o, n)) != null))) ? 1 : 0;

        public static int equalobj(lua_State L, TValue o1, TValue o2) => ((ttype(o1) == ttype(o2)) && (luaV_equalval(L, o1, o2) != 0)) ? 1 : 0;
        ...
        public static TValue luaV_tonumber(TValue obj, TValue n)
        {
            ...
            if (ttisstring(obj) && (luaO_str2d(svalue(obj), out var num) != 0))
            {
                setnvalue(n, num);
                return n;
            }
        ...
        {
            var mask = L.hookmask;
            var oldpc = InstructionPtr.Assign(L.savedpc);
        ...
        private static void callTMres(lua_State L, StkId res, TValue f, TValue p1, TValue p2)
        {
            var result = savestack(L, res);
        ...
        private static void callTM(lua_State L, TValue f, TValue p1, TValue p2, TValue p3)
        {
            setobj2s(L, L.top, f);  /* push function */
            setobj2s(L, L.top + 1, p1);  /* 1st argument */
            setobj2s(L, L.top + 2, p2);  /* 2nd argument */
        ...
        public static void luaV_gettable(lua_State L, TValue t, TValue key, StkId val)
        {
            for (int loop = 0; loop < MAXTAGLOOP; loop++)
            {
                ...
                {  /* `t' is a table? */
                    var h = hvalue(t);
                    var res = luaH_get(h, key); /* do a primitive get */
        ...
        public static void luaV_settable(lua_State L, TValue t, TValue key, StkId val)
        {
            ...
            for (int loop = 0; loop < MAXTAGLOOP; loop++)
            {
                ...
                {  /* `t' is a table? */
                    var h = hvalue(t);
                    var oldval = luaH_set(L, h, key); /* do a primitive set */
                ...
                setobj(L, vt, tm);  /* avoid pointing inside table (may rehash) */
                t = vt;
        ...
        private static int call_binTM(lua_State L, TValue p1, TValue p2,StkId res, TMS event_)
        {
            var tm = luaT_gettmbyobj(L, p1, event_);  /* try first operand */
            if (ttisnil(tm))
        ...
        private static TValue get_compTM(lua_State L, Table mt1, Table mt2, TMS event_)
        {
            var tm1 = fasttm(L, mt1, event_);
            if (tm1 == null) return null;  /* no metamethod */
            if (mt1 == mt2) return tm1;  /* same metatables => same metamethods */
            var tm2 = fasttm(L, mt2, event_);
        ...
        private static int call_orderTM(lua_State L, TValue p1, TValue p2, TMS event_)
        {
            var tm1 = luaT_gettmbyobj(L, p1, event_);
            if (ttisnil(tm1)) return -1;  /* no metamethod? */
            var tm2 = luaT_gettmbyobj(L, p2, event_);
        ...
        private static int l_strcmp(TString ls, TString rs)
        {
            ...
            for (;;)
            {
                ...
                int t = string.Compare(l.ToString(), r.ToString());
                if (t != 0) return t;
                else
        ...
        public static void luaV_concat(lua_State L, int total, int last)
        {
            do
            {
                ...
                else
                {
                    ...
                    var buffer = luaZ_openspace(L, G(L).buff, tl);
                    if (mybuff == null)
                        mybuff = buffer;
                    tl = 0;
                    for (i = n; i > 0; i--)
        ...
        public static void Arith(lua_State L, StkId ra, TValue rb, TValue rc, TMS op)
        {
            var tempb = new TValue();
            var tempc = new TValue();
            TValue b, c;
        ...
        internal static TValue RA(lua_State L, StkId base_, Instruction i) => base_ + GETARG_A(i);
        internal static TValue RB(lua_State L, StkId base_, Instruction i) => base_ + GETARG_B(i);
        internal static TValue RC(lua_State L, StkId base_, Instruction i) => base_ + GETARG_C(i);
        internal static TValue RKB(lua_State L, StkId base_, Instruction i, TValue[] k) => ISK(GETARG_B(i)) != 0 ? k[INDEXK(GETARG_B(i))] : base_ + GETARG_B(i);
        internal static TValue RKC(lua_State L, StkId base_, Instruction i, TValue[] k) => ISK(GETARG_C(i)) != 0 ? k[INDEXK(GETARG_C(i))] : base_ + GETARG_C(i);
        internal static TValue KBx(lua_State L, Instruction i, TValue[] k) => k[GETARG_Bx(i)];
        ...
        {
            var rb = RKB(L, base_, i, k);
            var rc = RKC(L, base_, i, k);
        ...
        reentry:  /* entry point */
            ...
            var pc = InstructionPtr.Assign(L.savedpc);
            var cl = clvalue(L.ci.func).l;
            var base_ = L.base_;
            var k = cl.p.k;
            ...
            {
                /*const*/
                var i = InstructionPtr.inc(ref pc)[0];
                StkId ra;
                ...
                switch (GET_OPCODE(i))
                {
                    ...
                    case OpCode.OP_FORLOOP:
                        {
                            var step = nvalue(ra + 2);
                            var idx = luai_numadd(nvalue(ra), step); /* increment index */
                            var limit = nvalue(ra + 1);
                    ...
                    case OpCode.OP_CLOSURE:
                        {
                            int j;
                            var p = cl.p.p[GETARG_Bx(i)];
                            var nup = p.nups;
                            var ncl = luaF_newLclosure(L, nup, cl.env);
                            ncl.l.p = p;
