...
namespace SharpLua
{
    ...
    public partial class Lua
    {
        ...
        {
            public lua_Number r = default;
            public TString ts = null;
            public SemInfo() { }
        ...
        public static bool currIsNewline(LexState ls) => (ls.current == '\n' || ls.current == '\r');
        ...
        public static void luaX_init(lua_State L)
        {
            ...
            {
                var ts = luaS_new(L, luaX_tokens[i]);
        ...
        {
            var L = ls.L;
            var ts = luaS_newlstr(L, str, l);
            var o = luaH_setstr(L, ls.fs.h, ts);  /* entry for `str' */
            if (ttisnil(o))
            {
        ...
        {
            var n = luaZ_bufflen(ls.buff);
            var p = luaZ_buffer(ls.buff);
            while ((n--) != 0)
                if (p[n] == from) p[n] = to;
        }
