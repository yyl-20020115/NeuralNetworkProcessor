...
namespace SharpLua
{
    ...
    {
        public Instruction[] codes = null;
        public int pc = -1;

        public InstructionPtr(Instruction[] codes = null, int pc = -1)
        {
            this.codes = codes; 
            this.pc = pc;
        }
        public static InstructionPtr Assign(InstructionPtr ptr) => 
            ptr == null ? null : new InstructionPtr(ptr.codes, ptr.pc);
        public Instruction this[int index]
        {
            get => this.codes[pc + index];
            set => this.codes[pc + index] = value;
        }
        public static InstructionPtr inc(ref InstructionPtr ptr) 
            => new InstructionPtr(ptr.codes, ptr.pc++);
    ...
    public partial class Lua
    {
		...
		*/
        ...
        public static InstructionPtr getcode(FuncState fs, expdesc e) 
            => new InstructionPtr(fs.f.code, e.u.s.info);
        ...
        private static int isnumeral(expdesc e) 
            => (e.k == expkind.VKNUM && e.t == NO_JUMP && e.f == NO_JUMP) ? 1 : 0;
        ...
        public static void luaK_nil(FuncState fs, int from, int n)
        {
            ...
            {  /* no jumps to current position? */
                ...
                {
                    var previous = new InstructionPtr(fs.f.code, fs.pc - 1);
        ...
        public static int luaK_jump(FuncState fs)
        {
            ...
            fs.jpc = NO_JUMP;
            var j = luaK_codeAsBx(fs, OpCode.OP_JMP, 0, NO_JUMP);
            luaK_concat(fs, ref j, jpc);  /* keep them on hold */
            return j;
        }


        public static void luaK_ret(FuncState fs, int first, int nret) => luaK_codeABC(fs, OpCode.OP_RETURN, first, nret + 1, 0);
        ...
        {
            var jmp = new InstructionPtr(fs.f.code, pc);
		...
		*/
        public static int luaK_getlabel(FuncState fs) => fs.lasttarget = fs.pc;
        ...
        {
            var pi = new InstructionPtr(fs.f.code, pc);
            return pc >= 1 && (testTMode(GET_OPCODE(pi[-1])) != 0)
                ? new InstructionPtr(pi.codes, pi.pc - 1)
                : new InstructionPtr(pi.codes, pi.pc);
        }
		...
		*/
        ...
        private static void exp2reg(FuncState fs, expdesc e, int reg)
        {
            ...
            if (hasjumps(e))
            {
                int final = 0;  /* position after whole expression */
        ...
        {
            var pc = getjumpcontrol(fs, e.u.s.info);
        ...
        public static void luaK_fixline(FuncState fs, int line) => fs.f.lineinfo[fs.pc - 1] = line;
