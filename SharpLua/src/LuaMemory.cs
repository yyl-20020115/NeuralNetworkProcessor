...
namespace SharpLua
{
    public partial class Lua
    {
        ...
        public static T[] luaM_reallocv<T>(lua_State L, T[] block, int new_size) 
            => (T[])luaM_realloc_(L, block, new_size);
        ...
        public static void luaM_freemem<T>(lua_State L, T b) 
		...
		*/
        ...
        public static T[] luaM_growaux_<T>(lua_State L, ref T[] block, ref int size, int limit, CharPtr errormsg)
        {
            T[] newblock = default;
            int newsize = 0;
            if (size >= limit / 2)
            ...
            newblock = luaM_reallocv(L, block, newsize);
		...
		*/
        ...
        {
            int unmanaged_size = GetUnmanagedSize(t);
        ...
        static void AddTotalBytes(lua_State L, int num_bytes)
            => G(L).totalbytes += (uint)num_bytes;
        static void SubtractTotalBytes(lua_State L, int num_bytes) 
            => G(L).totalbytes -= (uint)num_bytes;
