...
namespace SharpLua
{
    ...
    public partial class Lua
    {
        ...
        public static int savestack(lua_State L, StkId p) => p;
        public static StkId restorestack(lua_State L, int n) => L.stack[n];
        public static int saveci(lua_State L, CallInfo p) => p - L.base_ci;
        public static CallInfo restoreci(lua_State L, int n) => L.base_ci[n];
        ...
        public static void luaD_callhook(lua_State L, int event_, int line)
        {
            ...
            {
                var top = savestack(L, L.top);
                var ci_top = savestack(L, L.ci.top);
        ...
        private static StkId adjust_varargs(lua_State L, Proto p, int actual)
        {
            ...
            if ((p.is_vararg & VARARG_NEEDSARG) != 0)
        ...
        {
            /*const*/
            var tm = luaT_gettmbyobj(L, func, TMS.TM_CALL);
            ptrdiff_t funcr = savestack(L, func);
            if (!ttisfunction(tm))
                luaG_typeerror(L, func, "call");
            /* Open a hole inside the stack at `func' */
            for (var p = L.top; p > func; StkId.dec(ref p)) setobjs2s(L, p, p - 1);
            incr_top(L);
        ...
        public static int luaD_precall(lua_State L, StkId func, int nresults)
        {
            ...
            var funcr = savestack(L, func);
            var cl = clvalue(func).l;
            ...
            {  /* Lua function? prepare its call */
                ...
                var ci = inc_ci(L);  /* now `enter' new function */
                ci.func = func;
            ...
            {  /* if is a C function, call it */
                ...
                if ((L.hookmask & LUA_MASKCALL) != 0)
                    luaD_callhook(L, LUA_HOOKCALL, -1);
                lua_unlock(L);
                var n = curr_func(L).c.f(L);  /* do the actual call */
                lua_lock(L);
                if (n < 0)  /* yielding? */
                    return PCRYIELD;
                else
        ...
        {
            var fr = savestack(L, firstResult);  /* next call may change stack */
        ...
        public static int luaD_poscall(lua_State L, StkId firstResult)
        {
            ...
            var ci = CallInfo.dec(ref L.ci);
            var res = ci.func;  /* res == final position of 1st result */
            wanted = ci.nresults;
        ...
        public static int lua_resume(lua_State L, int nargs)
        {
            ...
            var status = luaD_rawrunprotected(L, resume, L.top - nargs);
            if (status != 0)
            {  /* error? */
                                      ...
                                      ptrdiff_t old_top, ptrdiff_t ef)
        {
            ...
            if (status != 0)
            {  /* an error occurred? */
                var oldtop = restorestack(L, old_top);
        ...
        private static void f_parser(lua_State L, object ud)
        {
            ...
            luaC_checkGC(L);
            var tf = (c == LUA_SIGNATURE[0]) ?
                luaU_undump(L, p.z, p.buff, p.name) :
                luaY_parser(L, p.z, p.buff, p.name);
            var cl = luaF_newLclosure(L, tf.nups, hvalue(gt(L)));
            cl.l.p = tf;
            for (var i = 0; i < tf.nups; i++)  /* initialize eventual upvalues */
        ...
        public static int luaD_protectedparser(lua_State L, ZIO z, CharPtr name)
        {
            ...
            {
                z = z,
                name = new CharPtr(name)
            };
            luaZ_initbuffer(L, p.buff);
            var status = luaD_pcall(L, f_parser, p, savestack(L, L.top), L.errfunc);
