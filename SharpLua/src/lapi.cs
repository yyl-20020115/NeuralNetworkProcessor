...
namespace SharpLua
{
    ...
    public partial class Lua
    {
        ...
        public static void lua_setlevel(lua_State from, lua_State to) => to.nCcalls = from.nCcalls;
		...
		*/
        ...
        public static int lua_gettop(lua_State L) => cast_int(L.top - L.base_);
        ...
        public static void lua_insert(lua_State L, int idx)
        {
            ...
            var p = index2adr(L, idx);
            api_checkvalidindex(L, p);
            for (var q = L.top; q > p; StkId.dec(ref q)) setobjs2s(L, q, q - 1);
            setobjs2s(L, p, L.top);
            lua_unlock(L);
        }
		...
		*/
        ...
        public static int lua_isstring(lua_State L, int idx) 
            => (lua_type(L, idx) is LUA_TSTRING or LUA_TNUMBER) ? 1 : 0;
        ...
        {
            var o1 = index2adr(L, index1);
            var o2 = index2adr(L, index2);
        ...
        public static int lua_equal(lua_State L, int index1, int index2)
        {
            ...
            var o1 = index2adr(L, index1);
            var o2 = index2adr(L, index2);
            var i = (o1 == luaO_nilobject || o2 == luaO_nilobject) ? 0 : equalobj(L, o1, o2);
            lua_unlock(L);
            return i;
        }
        ...
        public static int lua_lessthan(lua_State L, int index1, int index2)
        {
            ...
            var o1 = index2adr(L, index1);
            var o2 = index2adr(L, index2);
            var i = (o1 == luaO_nilobject || o2 == luaO_nilobject) ? 0
        ...
        public static lua_Integer lua_tointeger(lua_State L, int idx)
        {
            ...
            {
                var num = nvalue(o);
                lua_number2integer(out var res, num);
                return res;
            }
            else
                return 0;
		...
		*/
        ...
        public static CharPtr lua_pushvfstring(lua_State L, CharPtr fmt, object[] argp)
        {
            lua_lock(L);
            luaC_checkGC(L);
            var ret = luaO_pushvfstring(L, fmt, argp);
            lua_unlock(L);
            return ret;
        }
        ...
        public static CharPtr lua_pushfstring(lua_State L, CharPtr fmt)
        {
            lua_lock(L);
            luaC_checkGC(L);
            var ret = luaO_pushvfstring(L, fmt, null);
            lua_unlock(L);
            return ret;
        }
        ...
        public static CharPtr lua_pushfstring(lua_State L, CharPtr fmt, params object[] p)
        {
            lua_lock(L);
            luaC_checkGC(L);
            var ret = luaO_pushvfstring(L, fmt, p);
            lua_unlock(L);
            return ret;
        }
        ...
        public static void lua_pushcclosure(lua_State L, lua_CFunction fn, int n)
        {
            ...
            luaC_checkGC(L);
            api_checknelems(L, n);
            var cl = luaF_newCclosure(L, n, getcurrenv(L));
            cl.c.f = fn;
            L.top -= n;
            while (n-- != 0)
		...
		*/
        ...
        public static void checkresults(lua_State L, int na, int nr) => api_check(L, (nr) == LUA_MULTRET || (L.ci.top - L.top >= (nr) - (na)));
		...
		*/
        ...
        {
            var c = ud as CCallS;
            var cl = luaF_newCclosure(L, 0, getcurrenv(L));
            cl.c.f = c.func;
        ...
        public static int lua_cpcall(lua_State L, lua_CFunction func, object ud)
        {
            ...
            c.func = func;
            c.ud = ud;
            var status = luaD_pcall(L, f_Ccall, c, savestack(L, L.top), 0);
            lua_unlock(L);
            return status;
        }
                              ...
                              CharPtr chunkname)
        {
            ...
            var status = luaD_protectedparser(L, z, chunkname);
            lua_unlock(L);
            return status;
        }
        ...
        public static int lua_status(lua_State L) => L.status;
		...
		*/
        ...
        public static int lua_next(lua_State L, int idx)
        {
            ...
            var t = index2adr(L, idx);
            api_check(L, ttistable(t));
            var more = luaH_next(L, hvalue(t), L.top - 1);
            if (more != 0)
            {
                api_incr_top(L);
            }
        ...
        public static object lua_newuserdata(lua_State L, uint size)
        {
            lua_lock(L);
            luaC_checkGC(L);
            var u = luaS_newudata(L, size, getcurrenv(L));
            setuvalue(L, L.top, u);
            api_incr_top(L);
            lua_unlock(L);
            return u.user_data;
        ...
        internal static object lua_newuserdata(lua_State L, Type t)
        {
            lua_lock(L);
            luaC_checkGC(L);
            var u = luaS_newudata(L, t, getcurrenv(L));
            setuvalue(L, L.top, u);
            api_incr_top(L);
            lua_unlock(L);
            return u.user_data;
        ...
        public static CharPtr lua_getupvalue(lua_State L, int funcindex, int n)
        {
            var val = new TValue();
            lua_lock(L);
            var name = aux_upvalue(index2adr(L, funcindex), n, ref val);
            if (name != null)
            {
        ...
        public static CharPtr lua_setupvalue(lua_State L, int funcindex, int n)
        {
            var val = new TValue();
            lua_lock(L);
            var fi = index2adr(L, funcindex);
            api_checknelems(L, 1);
            var name = aux_upvalue(fi, n, ref val);
            if (name != null)
            {
                StkId.dec(ref L.top);
                setobj(L, val, L.top);
