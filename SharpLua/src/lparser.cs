...
namespace SharpLua
{
    ...
    public partial class Lua
    {
        ...
        public static int hasmultret(expkind k) => ((k) == expkind.VCALL || (k) == expkind.VVARARG) ? 1 : 0;

        public static LocVar getlocvar(FuncState fs, int i) => fs.f.locvars[fs.actvar[i]];
        ...
        private static void error_expected(LexState ls, int token) => luaX_syntaxerror(ls, luaO_pushfstring(ls.L, LUA_QS + " expected", luaX_token2str(ls, token)));

        private static void errorlimit(FuncState fs, int limit, CharPtr what)
        {
            var msg = (fs.f.linedefined == 0) ?
        ...
        private static void codestring(LexState ls, expdesc e, TString s) => init_exp(e, expkind.VK, luaK_stringK(ls.fs, s));
        ...
        public static void new_localvarliteral(LexState ls, CharPtr v, int n) => new_localvar(ls, luaX_newstring(ls, "" + v, (uint)(v.chars.Length - 1)), n);
        ...
        private static expkind singlevaraux(FuncState fs, TString n, expdesc _var, int base_)
        {
            if (fs == null)
            {  /* no more levels? */
                init_exp(_var, expkind.VGLOBAL, NO_REG);  /* default is global variable */
            ...
            else
            {
                ...
                if (v >= 0)
                {
                    init_exp(_var, expkind.VLOCAL, v);
                    if (base_ == 0)
                ...
                {  /* not found at current level; try upper one */
                    if (singlevaraux(fs.prev, n, _var, 0) == expkind.VGLOBAL)
                        return expkind.VGLOBAL;
                    _var.u.s.info = indexupvalue(fs, n, _var);  /* else was LOCAL or UPVAL */
                    _var.k = expkind.VUPVAL;  /* upvalue in this level */
        ...
        private static void pushclosure(LexState ls, FuncState func, expdesc v)
        {
            ...
            var f = fs.f;
            int oldsize = f.sizep;
            luaM_growvector(ls.L, ref f.p, fs.np, ref f.sizep, MAXARG_Bx, "constant table overflow");
