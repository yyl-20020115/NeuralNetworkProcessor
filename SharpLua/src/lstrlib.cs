...
namespace SharpLua
{
    ...
    public partial class Lua
    {
        ...
        {
            luaL_checklstring(L, 1, out var l);
        ...
        {
            var s = luaL_checklstring(L, 1, out var l);
        ...
        {
            var s = luaL_checklstring(L, 1, out var l1);
            var p = luaL_checklstring(L, 2, out var l2);
            var init = posrelat(luaL_optinteger(L, 3, 1), l1) - 1;
            if (init < 0) init = 0;
            ...
            {  /* or no special characters? */
                /* do a plain search */
                var s2 = lmemfind(s + init, (uint)(l1 - init), p, (uint)(l2));
                if (s2 != null)
                {
        ...
        private static int str_find(lua_State L) => str_find_aux(L, 1);
        ...
        {
            var ms = new MatchState();
            var s = lua_tolstring(L, lua_upvalueindex(1), out var ls);
            var p = lua_tostring(L, lua_upvalueindex(2));
            CharPtr src;
            ms.L = L;
            ms.src_init = s;
            ms.src_end = s + ls;
        ...
        private static int gfind_nodef(lua_State L) => luaL_error(L, LUA_QL("string.gfind") + " was renamed to " +
        ...
        {
            uint l, i;
            var news = lua_tolstring(ms.L, 3, out l);
            for (i = 0; i < l; i++)
            {
                if (news[i] != L_ESC)
        ...
        {
            var src = luaL_checklstring(L, 1, out var srcl);
        ...
        private static int str_format(lua_State L)
        {
            ...
            int arg = 1;
            var strfrmt = luaL_checklstring(L, arg, out var sfl);
