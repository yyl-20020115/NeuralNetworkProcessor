...
namespace SharpLua
{
    public static class Utils
    {
        ...
        public static string ReplaceMetaChars(string input)
            => Regex.Replace(input, @"(\\)(\d{3}|[^\d])?", new MatchEvaluator(ReplaceMetaCharsMatch));
        ...
        internal static Regex FormatParametersRegex = new (@"\%(\d*\$)?([\'\#\-\+ ]*)(\d*)(?:\.(\d+))?([hl])?([dioxXucsfeEgGpn%])");
        ...
        public static string sprintf(string Format, params object[] Parameters)
        {
            ...
            m = FormatParametersRegex.Match(builder.ToString());
            while (m.Success)
            {
                ...
                builder.Remove(m.Index, m.Length);
                builder.Insert(m.Index, w);
                m = FormatParametersRegex.Match(builder.ToString(), m.Index + w.Length);
            }
