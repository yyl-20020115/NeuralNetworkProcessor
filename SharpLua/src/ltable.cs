...
namespace SharpLua
{
    ...
    public partial class Lua
    {
		...
		*/
        internal static Node gnode(Table t, int i) => t.node[i];
        internal static TKey_nk gkey(Node n) => n.i_key.nk;
        internal static TValue gval(Node n) => n.i_val;
        internal static Node gnext(Node n) => n.i_key.nk.next;
        internal static void gnext_set(Node n, Node v) => n.i_key.nk.next = v;
        internal static TValue key2tval(Node n) => n.i_key.tvk;
		...
		*/
        ...
        internal static Node hashpow2(Table t, lua_Number n) => gnode(t, (int)lmod(n, sizenode(t)));

        public static Node hashstr(Table t, TString str) => hashpow2(t, str.tsv.hash);
        public static Node hashboolean(Table t, int p) => hashpow2(t, p);

        /*
		** for some types, it is better to avoid modulus by power of 2, as
		** they tend to have many 2 factors.
		*/
        public static Node hashmod(Table t, int n) => gnode(t, (n % ((sizenode(t) - 1) | 1)));

        public static Node hashpointer(Table t, object p) => hashmod(t, p.GetHashCode());
		...
		*/
        ...
        private static void setnodevector(lua_State L, Table t, int size)
        {
            ...
            else
            {
                ...
                size = twoto(lsize);
                var nodes = luaM_newvector<Node>(L, size);
                t.node = nodes;
        ...
        {
            int nasize, na;
            var nums = new int[MAXBITS + 1];  /* nums[i] = number of keys between 2^(i-1) and 2^i */
            int totaluse;
		...
		*/
        private static TValue newkey(lua_State L, Table t, TValue key)
        {
            ...
            {
                var n = getfreepos(t);  /* get a free place */
                if (n == null)
                ...
                var othern = mainposition(t, key2tval(mp));
                if (othern != mp)
