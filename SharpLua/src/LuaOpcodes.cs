...
namespace SharpLua
{
    ...
    public partial class Lua
    {
		...
		*/
        ...
        internal static uint MASK1(int n, int p) => (uint)((~((~0) << n)) << p);

        /* creates a mask with `n' 0 bits at position `p' */
        internal static uint MASK0(int n, int p) => (uint)(~MASK1(n, p));

        /*
		** the following macros help to manipulate instructions
		*/
        internal static OpCode GET_OPCODE(Instruction i) => (OpCode)((i >> POS_OP) & MASK1(SIZE_OP, 0));
        internal static OpCode GET_OPCODE(InstructionPtr i) => GET_OPCODE(i[0]);

        internal static void SET_OPCODE(ref Instruction i, Instruction o) => i = (Instruction)(i & MASK0(SIZE_OP, POS_OP)) | ((o << POS_OP) & MASK1(SIZE_OP, POS_OP));
        internal static void SET_OPCODE(ref Instruction i, OpCode opcode) => i = (Instruction)(i & MASK0(SIZE_OP, POS_OP)) | (((uint)opcode << POS_OP) & MASK1(SIZE_OP, POS_OP));
        internal static void SET_OPCODE(InstructionPtr i, OpCode opcode) => SET_OPCODE(ref i.codes[i.pc], opcode);

        internal static int GETARG_A(Instruction i) => (int)((i >> POS_A) & MASK1(SIZE_A, 0));
        internal static int GETARG_A(InstructionPtr i) => GETARG_A(i[0]);

        internal static void SETARG_A(InstructionPtr i, int u) => i[0] = (Instruction)((i[0] & MASK0(SIZE_A, POS_A)) | ((u << POS_A) & MASK1(SIZE_A, POS_A)));

        internal static int GETARG_B(Instruction i) => (int)((i >> POS_B) & MASK1(SIZE_B, 0));
        internal static int GETARG_B(InstructionPtr i) => GETARG_B(i[0]);
        ...
        => (int)((i >> POS_C) & MASK1(SIZE_C, 0));
        internal static int GETARG_C(InstructionPtr i) => GETARG_C(i[0]);

        internal static void SETARG_C(InstructionPtr i, int b) => i[0] = (Instruction)((i[0] & MASK0(SIZE_C, POS_C)) | ((b << POS_C) & MASK1(SIZE_C, POS_C)));

        internal static int GETARG_Bx(Instruction i) => (int)((i >> POS_Bx) & MASK1(SIZE_Bx, 0));
        internal static int GETARG_Bx(InstructionPtr i) => GETARG_Bx(i[0]);
        ...
        internal static int GETARG_sBx(InstructionPtr i) => GETARG_sBx(i[0]);

        internal static void SETARG_sBx(InstructionPtr i, int b)
        => SETARG_Bx(i, b + MAXARG_sBx);

        internal static int CREATE_ABC(OpCode o, int a, int b, int c) => (int)(((int)o << POS_OP) | (a << POS_A) | (b << POS_B) | (c << POS_C));
		...
		*/
        ...
        internal static int ISK(int x) => x & BITRK;

        /* gets the index of the constant */
        internal static int INDEXK(int r) => r & (~BITRK);

        internal static readonly int MAXINDEXRK = BITRK - 1;

        /* code a constant index as a RK value */
        internal static int RKASK(int x) => x | BITRK;
		...
		*/
        ...
        public static OpMode getOpMode(OpCode m) => (OpMode)(luaP_opmodes[(int)m] & 3);
        public static OpArgMask getBMode(OpCode m) => (OpArgMask)((luaP_opmodes[(int)m] >> 4) & 3);
        public static OpArgMask getCMode(OpCode m) => (OpArgMask)((luaP_opmodes[(int)m] >> 2) & 3);
        public static int testAMode(OpCode m) => luaP_opmodes[(int)m] & (1 << 6);
        public static int testTMode(OpCode m) => luaP_opmodes[(int)m] & (1 << 7);
        ...
        => (lu_byte)(((t) << 7) | ((a) << 6) | (((lu_byte)b) << 4) | (((lu_byte)c) << 2) | ((lu_byte)m));
