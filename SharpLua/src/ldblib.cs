...
namespace SharpLua
{
    public partial class Lua
    {
        ...
        private static int db_getinfo(lua_State L)
        {
            ...
            var L1 = getthread(L, out var arg);
            var options = luaL_optstring(L, arg + 2, "flnSu");
        ...
        private static int db_getlocal(lua_State L)
        {
            ...
            var name = lua_getlocal(L1, ar, luaL_checkint(L, arg + 2));
            if (name != null)
            {
                lua_xmove(L1, L, 1);
        ...
        private static int auxupvalue(lua_State L, int get)
        {
            ...
            var name = (get != 0) ? lua_getupvalue(L, 1, n) : lua_setupvalue(L, 1, n);
        ...
        private static int db_getupvalue(lua_State L) => auxupvalue(L, 1);
