...
namespace SharpLua
{
    ...
    public static class LuaCompiler
    {
        ...
        static Lua.Proto toproto(Lua.lua_State L, int i) => Lua.clvalue(L.top + (i)).l.p;
        ...
        static Lua.Proto combine(Lua.lua_State L, int n)
        {
            ...
            {
                int i, pc;
                var f = Lua.luaF_newproto(L);
                Lua.setptvalue2s(L, L.top, f); 
                Lua.incr_top(L);
                f.source = Lua.luaS_newliteral(L, "=(" + PROGNAME + ")");
                f.maxstacksize = 1;
                pc = 2 * n + 1;
                f.code = Lua.luaM_newvector<Instruction>(L, pc);
                f.sizecode = pc;
        ...
        static int writer(Lua.lua_State L, Lua.CharPtr p, uint size, object u) =>
            //UNUSED(L);
            ((Lua.fwrite(p, (int)size, 1, (Stream)u) != 1) && (size != 0)) ? 1 : 0;

        static int pmain_c(Lua.lua_State L)
        {
        ...
        static int _Main(string[] args)
        {
            ...
            s.argc = argc;
            s.argv = args;
            if (Lua.lua_cpcall(L, pmain_c, s) != 0)
            {
