...
namespace SharpLua
{
    public static class LuaProgram
    {
        ...
        static int docall(Lua.lua_State L, int narg, int clear)
        {
            ...
            int status = Lua.lua_pcall(L, narg, ((clear != 0) ? 0 : Lua.LUA_MULTRET), base_);
        ...
        static void print_version() => l_message(null, Lua.LUA_RELEASE + "  " + Lua.LUA_COPYRIGHT);
        ...
        static int incomplete(Lua.lua_State L, int status)
        {
            ...
            {
                var msg = Lua.lua_tolstring(L, -1, out var lmsg);
                var tp = msg + lmsg - (Lua.strlen(Lua.LUA_QL("<eof>")));
        ...
        static int pushline(Lua.lua_State L, int firstline)
        {
            ...
            var b = new Lua.CharPtr(buffer);
            var prmt = get_prompt(L, firstline);
        ...
        static int handle_script(Lua.lua_State L, string[] argv, int n)
        {
            ...
            Lua.CharPtr fname = argv[n];
            if (Lua.strcmp(fname, "-") == 0 && Lua.strcmp(argv[n - 1], "--") != 0)
                fname = null;  /* stdin */
            int status = Lua.luaL_loadfile(L, fname);
        ...
        {
            var init = Lua.getenv(Lua.LUA_INIT);
        ...
        {
            var s = Lua.lua_touserdata(L, 1) as MainParameters;
            var argv = s.argv;
            ...
            var script = collectargs(argv, ref has_i, ref has_v, ref has_e);
            if (script < 0)
            {  /* invalid args? */
                print_usage();
                s.status = 1;
        ...
        static int Main(string[] args)
        {
            ...
            var L = Lua.lua_open();  /* create state */
            if (L == null)
            {
                l_message(args[0], "cannot create state: not enough memory");
                return Lua.EXIT_FAILURE;
            }
            var s = new MainParameters
            {
                argc = args.Length,
                argv = args
            };
            var status = Lua.lua_cpcall(L, pmain, s);
            report(L, status);
            Lua.lua_close(L);
            return (status != 0) || (s.status != 0) 
                ? Lua.EXIT_FAILURE 
                : Lua.EXIT_SUCCESS
                ;
        }
