...
public class CInterpreter
{
    ...
    public Value ReadMemory (int address) => Stack[address];

    public Value WriteMemory (int address, Value value) => Stack[address] = value;
    ...
    public string ReadString (int address) => ReadStringWithEncoding (address, Encoding.UTF8);
    ...
    public void Return ()
    {
        ...
        // Pop the stack
        //
        var frame = ActiveFrame ?? throw new InvalidOperationException ($"Cannot call Return with no ActiveFrame");
