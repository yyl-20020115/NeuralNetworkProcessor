...
public abstract class EmitContext
{
    ...
    public virtual CType ResolveTypeName (TypeName typeName) => MakeCType (typeName.Specifiers, typeName.Declarator, null, new Block (VariableScope.Global));
    ...
    public virtual ResolvedVariable ResolveMethodFunction (CStructType structType, CStructMethod method)
    {
        ...
        return r != null ? r : throw new Exception ("Cannot resolve method function");
    }
    ...
    public virtual Label DefineLabel () => ParentContext != null ? ParentContext.DefineLabel () : new Label ();

    public virtual void EmitLabel (Label l) => ParentContext?.EmitLabel (l);
    ...
    public void EmitCast (CType fromType, CType toType)
    {
        ...
        if (fromBasicType != null && toType is CBasicType toBasicType) {
    ...
    public virtual Value GetConstantMemory (string stringConstant) => ParentContext != null
            ? ParentContext.GetConstantMemory (stringConstant)
    ...
    CType MakeCType (CType type, Declarator? decl, Initializer? init, Block? block)
    {
        ...
        else if (decl is PointerDeclarator pdecl) {
    ...
    public CType MakeCType (DeclarationSpecifiers specs, Initializer? init, Block? block)
    {
        ...
        if (specs.StorageClassSpecifier == StorageClassSpecifier.Auto) {
            if (init is not ExpressionInitializer einit) {
        ...
        if (basicTs != null) {
            if (basicTs.Name == "void") {
                ...
                var type = typeName == "float" ? CBasicType.Float
                    : (typeName == "double" ? CBasicType.Double
