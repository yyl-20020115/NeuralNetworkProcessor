...
public class Address : CodeArray, IOperand
{
    ...
    public Reg ToReg() => throw new NotImplementedException();
}
public class AddressFrame
{
    public AddressFrame(int bit) => Bit = bit;
    ...
    public Address this[RegRip addr]
    {
        get
        {
            Address frame = new(Bit, true, addr.Disp, false);
            frame.Db((int)0b00000101);
            frame.Dd(addr.Disp);
            return frame;
        }
    }

    public Address this[ulong disp] => new(64, true, disp, false, true);
