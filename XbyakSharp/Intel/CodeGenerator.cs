...
public partial class CodeGenerator : CodeArray, ICodeGenerator
{
    ...
    private static bool IsXMM_XMMorMEM(IOperand op1, IOperand op2) => op1.IsXMM() && (op2.IsXMM() || op2.IsMEM());

    private static bool IsXMMorMMX_MEM(IOperand op1, IOperand op2) => (op1.IsMMX() && (op2.IsMMX() || op2.IsMEM())) || IsXMM_XMMorMEM(op1, op2);

    private static bool IsXMM_MMXorMEM(IOperand op1, IOperand op2) => op1.IsXMM() && (op2.IsMMX() || op2.IsMEM());

    private static bool IsMMX_XMMorMEM(IOperand op1, IOperand op2) => op1.IsMMX() && (op2.IsXMM() || op2.IsMEM());

    private static bool IsXMM_REG32orMEM(IOperand op1, IOperand op2) => op1.IsXMM() && (op2.IsREG(i32e) || op2.IsMEM());

    private static bool IsREG32_XMMorMEM(IOperand op1, IOperand op2) => op1.IsREG(i32e) && (op2.IsXMM() || op2.IsMEM());
    ...
    private void Rex(IOperand op1) => Rex(op1, new Operand());
    ...
    private bool IsInDisp16(uint x) => 0xFFFF8000 <= x || x <= 0x7FFF;

    private byte GetModRM(int mod, int r1, int r2) => unchecked((byte)((mod << 6) | ((r1 & 7) << 3) | (r2 & 7)));
    ...
    private void OpExt(IOperand op, Mmx mmx, int code, int imm, bool hasMMX2 = false)
    {
        if (hasMMX2 && op.IsREG(i32e))
        {
            ...
            OpModR(op.ToReg(), mmx, 0x0F, (int)0b11000101);
            Db(imm);
        }
        else
        {
            OpGen(mmx, op, code, 0x66, IsXMM_REG32orMEM, imm, (int)0b00111010);
        }
    }
    ...
    {
        VerifyMemHasSize(op);
        OpR_ModM(op, 0, ext, ((int)0b11000000 | ((imm == 1 ? 1 : 0) << 4)));
        if (imm != 1)
        {
            Db(imm);
        }
    ...
    private void OpShift(IOperand op, Reg8 cl, int ext)
    {
        ...
        OpR_ModM(op, 0, ext, (int)0b11010010);
    }
    ...
    private void OpRM_I(IOperand op, uint imm, int code, int ext)
    {
        ...
        {
            OpR_ModM(op, 0, ext, (int)0b10000000 | (immBit < Math.Min(op.Bit, 32) ? 2 : 0));
        }
        Db(imm, (int)(immBit / 8));
    }
    ...
    private void OpIncDec(IOperand op, int code, int ext)
    {
        ...
        code = (int)0b11111110;
        Reg tr = new (ext, Operand.KindType.REG, op.Bit);
        if (op.IsREG())
        {
    ...
    public bool HasUndefinedLabel() => label.HasUndefinedLabel();
    ...
    public T GetDelegate<T>() where T : class
        => Marshal.GetDelegateForFunctionPointer(CodeMemory.DangerousGetHandle(), typeof(T)) as T;
